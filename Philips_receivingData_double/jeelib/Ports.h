...
protected:
    ...
    inline uint8_t digiPin() const
        { 
		if ( portNum == 5 ) return 6;//special for life jacket
		if ( portNum == 6 ) return 7;//special for life jacket
		return portNum ? portNum + 3 : 18; 
		}
	/// @return Arduino digital pin number of a Port's A pin (uint8_t).
    inline uint8_t digiPin2() const
        { 
		if ( portNum == 5 ) return 9;//special for life jacket
		if ( portNum == 6 ) return 8;//special for life jacket
